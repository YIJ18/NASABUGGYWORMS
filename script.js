...
function init() {
    ...
    loadPlanets();
    loadComets();
    loadAsteroids(); // Cargar asteroides
...
// Función para cargar asteroides desde la base de datos
function loadAsteroids() {
    fetch('get_asteroids.php') // Cambia esto por la URL correcta de tu API
        .then(response => response.json())
        .then(data => {
            data.asteroids.forEach((data, index) => {
                const asteroid = createAsteroid({
                    ...
                    perihelion: data.perihelion,
                    inclination: data.inclination ? data.inclination : 0, // Inclinación del asteroide
                    eccentricity: data.eccentricity ? data.eccentricity : 0.5, // Excentricidad de la órbita
                    index: index // Índice para identificar la trayectoria del asteroide
                });
...
function createAsteroid({ diameter, position, perihelion, inclination = 0, eccentricity = 0.5, index }) {
    const geometry = new THREE.SphereGeometry(diameter, 16, 16);
    const material = new THREE.MeshBasicMaterial({ color: 0x00ff00 }); // Color verde para los asteroides
    const asteroid = new THREE.Mesh(geometry, material);

    asteroid.position.set(position.x, position.y, position.z);
    asteroid.perihelion = perihelion;
    asteroid.inclination = inclination;
    asteroid.eccentricity = eccentricity;
    asteroid.index = index; // Añadimos el índice del asteroide
...
function animateAsteroids() {
    const speed = 0.0001; // Ajusta la velocidad de los asteroides

    asteroidPaths.forEach((asteroidPath) => {
        const asteroid = asteroidPath.asteroid;
        const time = Date.now() * speed;

        // Calcula la posición en la órbita elíptica
        const a = asteroid.perihelion;
        const b = asteroid.perihelion * Math.sqrt(1 - asteroid.eccentricity ** 2);

        asteroid.position.x = a * Math.cos(time);
        asteroid.position.z = b * Math.sin(time);
        asteroid.position.y = asteroid.position.z * Math.tan(THREE.Math.degToRad(asteroid.inclination));

        // Agregar el punto actual a la trayectoria
        asteroidPath.points.push(asteroid.position.clone());

        // Limitar la cantidad de puntos para evitar acumulaciones
        if (asteroidPath.points.length > 1000) {
            asteroidPath.points.shift(); // Elimina el primer punto para mantener la longitud
        }

        // Actualizar o dibujar los puntos en la órbita
        if (asteroidPath.particles) {
            asteroidPath.particles.geometry.setFromPoints(asteroidPath.points);
        } else {
            const geometry = new THREE.BufferGeometry().setFromPoints(asteroidPath.points);
            const material = new THREE.PointsMaterial({ color: 0x00ff00, size: 0.5 });
            const particles = new THREE.Points(geometry, material);
            asteroidPath.particles = particles;
            scene.add(particles);
        }
    });
}
